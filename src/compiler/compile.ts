...
function buildAst(cst: CstNode): ShastaNode {
  ...
  function cstNodeToShastaNode(
    ...
    if ("propertyAccess" in children) {
      return {
        type: "identifier",
        name: (
          (children.propertyAccess[0] as CstNode).children
            .Identifier as IToken[]
        )
  ...
  return cstNodeToShastaNode(cst);
}
...
export function compile(source: string) {
  ...
  const intermediate = buildAst(cst);
